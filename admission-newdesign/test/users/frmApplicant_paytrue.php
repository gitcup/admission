...
<h2>พิมพ์เอกสารหลักฐานการสมัคร ผู้สมัครชำระเงินแล้ว </h2>
<form id="form1" name="form1" method="post" action="index.php?p=frmApplicant_paytrue">
  ปีการศึกษา 
  <select name="lstacadyear" id="lstacadyear">
    <option value="2562">2562</option>
    <?
                        $strSQL = "SELECT * FROM AVSREG.Z_ACADYEAR where id > 2558 ORDER BY id";
...
<table width="950" border="0" align="left" cellpadding="0" cellspacing="0">
  <tr>
	...
	$strSEL .= " AND (((APPLICANT.APPLICANTMETHOD)='W') AND ((APPLICANT.APPLICANTSTATUS)>='10') AND ((APPLICANT.FINANCESTATUS)='N') AND ((APPLICANTSELECTION.SEQUENCE)='1') AND ((QUOTASTATUS.STUDYPERIOD)='$studyperiod') AND ((VOUCHER.VOUCHERSTATUS)<>'V')) ORDER BY VOUCHER.VOUCHERDATE, APPLICANT.CREATEDATETIME ASC";		
	...
	$strSEL1 .= " AND(((APPLICANT.APPLICANTMETHOD)='W') AND ((APPLICANT.APPLICANTSTATUS)>='10') AND ((APPLICANT.FINANCESTATUS)='N') AND ((APPLICANTSELECTION.SEQUENCE)='1') AND ((QUOTASTATUS.STUDYPERIOD)='$studyperiod') AND ((VOUCHER.VOUCHERSTATUS)<>'V')) ORDER BY VOUCHER.VOUCHERDATE, APPLICANT.CREATEDATETIME ASC) bc1  where rownum <= $goto ) where rnum > $pagelen2";
...
<p> <?
 ...
 echo "<strong>ทั้งหมด : </strong>  $totalrecords  คน/ จำนวน $totalpage หน้า <br/>"; 
	...
	{
		$back = $npage - 1;
		echo ""." <a href=\"$pathPresent?p=frmApplicant_paytrue&npage=1&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code\" >แรกสุด</a> ";
		echo " <a href=\"$pathPresent?p=frmApplicant_paytrue&npage=$back&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code\" >ก่อนหน้า</a> ";
		if ($start > 1) { echo "..."; }
	}
	...
	For ($i=$start ; $i<=$end ; $i++) 
	{
		if ($i == $npage ) 
		{
			...
			echo " <strong> $i </strong> ";
				 ...
				 echo " <a href=\"$pathPresent?p=frmApplicant_paytrue&npage=$i&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code\"> $i </a> ";
		 	   }		   
	}
	...
	if ($npage < $totalpage) 
	{
		...
		echo " <a href='$pathPresent?p=frmApplicant_paytrue&npage=$next&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code'>ถัดไป</a> ";
		echo " <a href='$pathPresent?p=frmApplicant_paytrue&npage=$totalpage&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code' >ท้ายสุด</a> ";
	}
	?>
&nbsp;</p>
