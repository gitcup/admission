...
<form id="form1" name="form1" method="post" action="index.php?p=frmApplicant_paytrueexce">
  ปีการศึกษา 
...
<p> <?
 ...
 echo "<strong>ทั้งหมด : </strong>  $totalrecords  คน/ จำนวน $totalpage หน้า <br/>"; 
	...
	{
		$back = $npage - 1;
		echo ""." <a href=\"$pathPresent?p=frmApplicant_paytrueexce&npage=1&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code\" >แรกสุด</a> ";
		echo " <a href=\"$pathPresent?p=frmApplicant_paytrueexce&npage=$back&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code\" >ก่อนหน้า</a> ";
		if ($start > 1) { echo "..."; }
	}
	...
	For ($i=$start ; $i<=$end ; $i++) 
	{
		if ($i == $npage ) 
		{
			...
			echo " <strong> $i </strong> ";
				 ...
				 echo " <a href=\"$pathPresent?p=frmApplicant_paytrueexce&npage=$i&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code\"> $i </a> ";
		 	   }		   
	}
	...
	if ($npage < $totalpage) 
	{
		...
		echo " <a href='$pathPresent?p=frmApplicant_paytrueexce&npage=$next&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code'>ถัดไป</a> ";
		echo " <a href='$pathPresent?p=frmApplicant_paytrueexce&npage=$totalpage&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code' >ท้ายสุด</a> ";
	}
	?>
&nbsp;</p>
