...
<h2>ผู้สมัครที่ผ่านการตรวจคุณสมบัติ (ผ่าน 
  <input name="imageField2" type="image" src="../images/icon_true.png" alt="ผ่าน" />
) <? echo"$txt"; ?></h2>
<form id="form1" name="form1" method="post" action="index.php?p=frmApplicant_checkPass">
  ปีการศึกษา 
  <select name="lstacadyear" id="lstacadyear">
    <?
                        $strSQL = "SELECT * FROM AVSREG.Z_ACADYEAR where id > 2552 ORDER BY id";
...
<select name="lstsemester" id="lstsemester">
 ...
 กรองรหัส
 <input name="txt_code" type="text" id="txt_code" value="<? echo $re_code; ?>" size="3" maxlength="1" /> 
...
<table width="950" border="0" align="left" cellpadding="0" cellspacing="0">
  <tr>
	...
	$strSEL = "SELECT APPLICANT.APPLICANTID,APPLICANT.APPLICANTCODE, PREFIX.PREFIXNAME, APPLICANT.APPLICANTNAME, APPLICANT.APPLICANTSURNAME, APPLICANT.GPAX, APPLICANT.CREATEDATETIME, APPLICANT.APPLICANTMETHOD, APPLICANT.EDITSEQ, APPLICANTDOC.DOCX,APPLICANTDOC.DOCY ,APPLICANTDOC.PROBLEM FROM (AVSREG.APPLICANT INNER JOIN AVSREG.PREFIX ON APPLICANT.PREFIXID = PREFIX.PREFIXID) INNER JOIN AVSREG.APPLICANTDOC ON APPLICANT.APPLICANTID = APPLICANTDOC.APPLICANTID ";
	 $first_where = 0;
		...
		{	
			...
			$sqlr .= "APPLICANT.ACADYEAR = '$re_acadyear' \n";
		}		
		// &#3611;&#3637;
		...
		{	
			...
			$sqlr .= "APPLICANT.SEMESTER = '$re_semester' \n";
		}		
		// &#3611;&#3619;&#3632;&#3648;&#3616;&#3607; &#3609;&#3624;.
		...
		{	
			...
			$sqlr .= "APPLICANT.APPLICANTTYPE = '$re_type' \n";
		}		
		// &#3627;&#3621;&#3633;&#3585;&#3626;&#3641;&#3605;&#3619;
		...
		{	
			...
			$sqlr .= "APPLICANT.APPLICANTCODE LIKE '%$re_code%' \n";
		}		
	...
	$strSEL1 = "SELECT * from(select rownum rnum, bc1.* from(SELECT APPLICANT.APPLICANTID,APPLICANT.APPLICANTCODE, PREFIX.PREFIXNAME, APPLICANT.APPLICANTNAME, APPLICANT.APPLICANTSURNAME, APPLICANT.GPAX, APPLICANT.CREATEDATETIME, APPLICANT.HOMEPHONENO, APPLICANT.APPLICANTMETHOD, APPLICANT.EDITSEQ, APPLICANTDOC.DOCX,APPLICANTDOC.DOCY ,APPLICANTDOC.PROBLEM, APPLICANTDOC.CHKDATE FROM (AVSREG.APPLICANT INNER JOIN AVSREG.PREFIX ON APPLICANT.PREFIXID = PREFIX.PREFIXID) INNER JOIN AVSREG.APPLICANTDOC ON APPLICANT.APPLICANTID = APPLICANTDOC.APPLICANTID WHERE (((APPLICANT.APPLICANTMETHOD)='W') AND ((APPLICANT.ACADYEAR)='$re_acadyear') AND ((APPLICANT.SEMESTER)='$re_semester') AND ((APPLICANT.APPLICANTTYPE)='$re_type') AND ((APPLICANT.ROUND)='$re_round') AND ((APPLICANT.APPLICANTSTATUS)='10')) ORDER BY APPLICANT.CREATEDATETIME ASC) bc1  where rownum <= $goto ) where rnum > $pagelen2";
        ...
        <td bgcolor="#FFFFFF">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr><form action="index.php?p=frmApplicant_check" method="post" name="form2" class="style7" id="form2">
              <td><div align="center"><? echo "<a href='#' onClick=\"TINY.box.show({iframe:'problem.php?check=no&appid=$applicantid',boxid:'frameless',width:695,height:450,fixed:false,maskid:'bluemask',maskopacity:40,closejs:function(){closeJS()}})\"><img src='../images/icon_false.png' alt='ไม่ผ่าน' border='0'></a>";?></div></td>
            </form>
              </tr>
          </table>        </td>
      </tr>
...
<p> <?
 ...
 echo "<strong>ทั้งหมด : </strong>  $totalrecords  คน/ จำนวน $totalpage หน้า <br/>"; 
	...
	{
		$back = $npage - 1;
		echo ""." <a href=\"$pathPresent?p=frmApplicant_checkPass&npage=1&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round\" >แรกสุด</a> ";
		echo " <a href=\"$pathPresent?p=frmApplicant_checkPass&npage=$back&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round\" >ก่อนหน้า</a> ";
		if ($start > 1) { echo "..."; }
	}
	...
	For ($i=$start ; $i<=$end ; $i++) 
	{
		if ($i == $npage ) 
		{
			...
			echo " <strong> $i </strong> ";
				 ...
				 echo " <a href=\"$pathPresent?p=frmApplicant_checkPass&npage=$i&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round\"> $i </a> ";
		 	   }		   
	}
	...
	if ($npage < $totalpage) 
	{
		...
		echo " <a href='$pathPresent?p=frmApplicant_checkPass&npage=$next&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round'>ถัดไป</a> ";
		echo " <a href='$pathPresent?p=frmApplicant_checkPass&npage=$totalpage&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round' >ท้ายสุด</a> ";
	}
	?>
&nbsp;</p>
