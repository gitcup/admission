...
if($line == '1') {
$strSQL = "SELECT QUOTA.QUOTAID, QUOTA.QUOTACODE, QUOTA.QUOTANAME, LEVELID.LEVELID, LEVELID.LEVELABB, QUOTASTATUS.QUOTASTATUSID, QUOTASTATUS.STUDYPERIOD, QUOTA.FACULTYID, FACULTY.FACULTYNAME, QUOTASTATUS.TOTALSEAT, QUOTASTATUS.SELECTIONSEAT, QUOTASTATUS.MINGPAX FROM (avsreg.FACULTY INNER JOIN (avsreg.QUOTA INNER JOIN avsreg.QUOTASTATUS ON QUOTA.QUOTAID = QUOTASTATUS.QUOTAID) ON FACULTY.FACULTYID = QUOTA.FACULTYID) INNER JOIN avsreg.LEVELID ON QUOTA.LEVELID = LEVELID.LEVELID
WHERE (((QUOTASTATUS.ACADYEAR)='$acadyear') AND ((QUOTASTATUS.SEMESTER)='$semester') AND ((QUOTASTATUS.APPLICANTTYPE)='$applicanttype')  AND ((QUOTASTATUS.ROUND)='$round') AND ((QUOTASTATUS.QUOTASTATUS)=10)) order by QUOTA.FACULTYID, LEVELID.LEVELID, QUOTA.QUOTACODE asc";
				} else {
				
				$strSQL = "SELECT QUOTA.QUOTAID, QUOTA.QUOTACODE, QUOTA.QUOTANAME, LEVELID.LEVELID, LEVELID.LEVELABB, QUOTASTATUS.QUOTASTATUSID, QUOTASTATUS.STUDYPERIOD, QUOTA.FACULTYID, FACULTY.FACULTYNAME, QUOTASTATUS.TOTALSEAT, QUOTASTATUS.SELECTIONSEAT, QUOTASTATUS.MINGPAX FROM (avsreg.FACULTY INNER JOIN (avsreg.QUOTA INNER JOIN avsreg.QUOTASTATUS ON QUOTA.QUOTAID = QUOTASTATUS.QUOTAID) ON FACULTY.FACULTYID = QUOTA.FACULTYID) INNER JOIN avsreg.LEVELID ON QUOTA.LEVELID = LEVELID.LEVELID
WHERE (((QUOTASTATUS.ACADYEAR)='$acadyear') AND ((QUOTASTATUS.SEMESTER)='$semester') AND ((QUOTASTATUS.APPLICANTTYPE)='$applicanttype') AND ((QUOTA.FACULTYID)<>'2') AND ((QUOTASTATUS.ROUND)='$round') AND ((QUOTASTATUS.QUOTASTATUS)=10)) order by QUOTA.FACULTYID asc";
...
<table width="600" border="1" cellpadding="3" cellspacing="1" class="table" id="menutype">
  <tr>
    <th width="550"> <div align="center">สาขาวิชา </div></th>
...
<?php $fac = $objResult["FACULTYID"];?>
	<?php if($tmpfac <> $fac) { ?>
  <tr>
    <td colspan="4"  class="table-primary" style="color: #004085; ">&nbsp;คณะ<?php=iconv("TIS-620", "UTF-8",$objResult["FACULTYNAME"]);?></td>
    
  </tr>
  <?php  }  ?>
  <tr>
  <?php  $q = $objResult["QUOTASTATUSID"];
  	  $studyperiod = $objResult["STUDYPERIOD"];
   ?>
	...
	<?php $sql = "SELECT APPLICANTSELECTION.APPLICANTID FROM avsreg.APPLICANTSELECTION where APPLICANTSELECTION.QUOTASTATUSID = '$q' and APPLICANTSELECTION.SEQUENCE = '1'";
		...
		while(odbc_fetch_row($objExecnumr))$Num_Rows++; // Count Record
		 ?>
    <td align="right" bgcolor="#FFFFFF"><?php=$objResult["TOTALSEAT"];?></td>
   <td align="right" bgcolor="#FFFFFF"><?php echo $Num_Rows;?></td>
    <?php $gpa = $objResult["MINGPAX"];?>
	<?php $levelid = $objResult["LEVELID"];?>
    <td bgcolor="#FFFFFF"><div align="center">
      <?php if($studyperiod == '2') { echo "ภาคพิเศษ (เสาร์-อาทิตย์)";} else { echo "ภาคปกติ (จันทร์-ศุกร์)";} ?>
    </div></td>
  </tr>
<?php
$tmpfac = $fac;
