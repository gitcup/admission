...
<h2>ผู้สมัครที่ไม่ผ่านการตรวจคุณสมบัติ (ไม่ผ่าน 
  <input name="imageField2" type="image" src="../images/icon_false.png" alt="ผ่าน" />
) <? echo"$txt"; ?></h2>
<form id="form1" name="form1" method="post" action="index.php?p=frmApplicant_checkunPass">
  ปีการศึกษา 
...
<table width="950" border="0" align="left" cellpadding="0" cellspacing="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="5">
      <tr>
        ...
        <td width="15%" bgcolor="#0099CC"><div align="center">วันที่สมัคร/วันที่ตรวจ</div></td>
        <td colspan="2" bgcolor="#0099CC"><div align="center">บัตรปชช/<br />
          ใบแสดงผลการเรียน</div>          <div align="center"></div></td>
        <td width="11%" bgcolor="#0099CC"><div align="center">Tel.</div></td>
        <td width="8%" bgcolor="#0099CC"><div align="center">เปลี่ยนสถานะ<br />
        เป็นผ่าน</div></td>
      </tr>
	  <?   
	  $pagelen = 20; 
	...
	$strSEL .= " AND (((APPLICANT.APPLICANTMETHOD)='W') AND ((APPLICANT.APPLICANTSTATUS)='9')) ORDER BY APPLICANT.CREATEDATETIME ASC";
	...
	$strSEL1 .= " AND(((APPLICANT.APPLICANTMETHOD)='W') AND ((APPLICANT.APPLICANTSTATUS)='9')) ORDER BY APPLICANT.CREATEDATETIME ASC) bc1  where rownum <= $goto ) where rnum > $pagelen2";
  ...
  ?>
	...
	<?   }  ?>
        ...
        <td bgcolor="#FFFFFF"><span class="style7">&nbsp;<? echo datethai($datetime); ?> /<br> <? if($chkdate <> "") { echo datethai($chkdate); } else { echo "-"; }?><br>
        </span></td>
        <td width="7%" bgcolor="#FFFFFF"><div align="center" class="style7"><? echo "<a href='$path_attachFile$docx' target='_blank'>ดูไฟล์</a>";?></div></td>
        <td width="9%" bgcolor="#FFFFFF"><div align="center" class="style7"><? echo "<a href='$path_attachFile$docy' target='_blank'>ดูไฟล์</a>";?></div></td>
...
<p> <?
 ...
 echo "<strong>ทั้งหมด : </strong>  $totalrecords  คน/ จำนวน $totalpage หน้า <br/>"; 
	...
	{
		$back = $npage - 1;
		echo ""." <a href=\"$pathPresent?p=frmApplicant_checkunPass&npage=1&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round\" >แรกสุด</a> ";
		echo " <a href=\"$pathPresent?p=frmApplicant_checkunPass&npage=$back&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round\" >ก่อนหน้า</a> ";
		if ($start > 1) { echo "..."; }
	}
	...
	For ($i=$start ; $i<=$end ; $i++) 
	{
		if ($i == $npage ) 
		{
			...
			echo " <strong> $i </strong> ";
				 ...
				 echo " <a href=\"$pathPresent?p=frmApplicant_checkunPass&npage=$i&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round\"> $i </a> ";
		 	   }		   
	}
	...
	if ($npage < $totalpage) 
	{
		...
		echo " <a href='$pathPresent?p=frmApplicant_checkunPass&npage=$next&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round'>ถัดไป</a> ";
		echo " <a href='$pathPresent?p=frmApplicant_checkunPass&npage=$totalpage&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round' >ท้ายสุด</a> ";
	}
	?>
&nbsp;</p>
