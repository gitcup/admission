...
<h2>ตรวจหลักฐานข้อมูลผู้สมัคร <? echo"$txt"; ?></h2>
<form id="form1" name="form1" method="post" action="index.php?p=frmApplicant_check">
  ปีการศึกษา 
...
<table width="950" border="0" align="left" cellpadding="0" cellspacing="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="5">
      <tr>
        ...
        <td width="17%" bgcolor="#0099CC"><div align="center">ชื่อ - สกุล </div></td>
        <td width="19%" bgcolor="#0099CC"><div align="center">สาขาที่เลือก</div></td>
        <td width="5%" bgcolor="#0099CC"><div align="center">GPA</div></td>
        <td width="13%" bgcolor="#0099CC"><div align="center">วันที่สมัคร/เบอร์โทร</div></td>
        <td colspan="2" bgcolor="#0099CC"><div align="center">บัตรปชช/<br />
          ใบแสดงผลการเรียน</div>          <div align="center"></div></td>
        <td width="7%" bgcolor="#0099CC"><div align="center">แก้ไข<br />
          ครั้งที่</div></td>
        <td width="14%" bgcolor="#0099CC"><div align="center">สถานะ<br />
        ผ่าน/ไม่ผ่าน</div></td>
      </tr>
	  <?   
	  $pagelen = 20; 
	...
	$strSEL = "SELECT APPLICANT.APPLICANTID,APPLICANT.APPLICANTCODE, PREFIX.PREFIXNAME, APPLICANT.APPLICANTNAME, APPLICANT.APPLICANTSURNAME, APPLICANT.GPAX, APPLICANT.CREATEDATETIME, APPLICANT.APPLICANTMETHOD, APPLICANT.EDITSEQ, APPLICANTDOC.DOCX,APPLICANTDOC.DOCY ,APPLICANTDOC.PROBLEM FROM (AVSREG.APPLICANT INNER JOIN AVSREG.PREFIX ON APPLICANT.PREFIXID = PREFIX.PREFIXID) INNER JOIN AVSREG.APPLICANTDOC ON APPLICANT.APPLICANTID = APPLICANTDOC.APPLICANTID WHERE (((APPLICANT.APPLICANTMETHOD)='W') AND ((APPLICANT.ACADYEAR)='$re_acadyear') AND ((APPLICANT.SEMESTER)='$re_semester') AND ((APPLICANT.APPLICANTTYPE)='$re_type') AND ((APPLICANT.ROUND)='$re_round') AND ((APPLICANT.APPLICANTSTATUS)='5')) ORDER BY APPLICANT.CREATEDATETIME ASC";
	...
	$strSEL1 = "SELECT * from(select rownum rnum, bc1.* from(SELECT APPLICANT.APPLICANTID,APPLICANT.APPLICANTCODE, PREFIX.PREFIXNAME, APPLICANT.APPLICANTNAME, APPLICANT.APPLICANTSURNAME, APPLICANT.GPAX, APPLICANT.CREATEDATETIME, APPLICANT.HOMEPHONENO, APPLICANT.APPLICANTMETHOD, APPLICANT.EDITSEQ, APPLICANTDOC.DOCX,APPLICANTDOC.DOCY ,APPLICANTDOC.PROBLEM FROM (AVSREG.APPLICANT INNER JOIN AVSREG.PREFIX ON APPLICANT.PREFIXID = PREFIX.PREFIXID) INNER JOIN AVSREG.APPLICANTDOC ON APPLICANT.APPLICANTID = APPLICANTDOC.APPLICANTID WHERE (((APPLICANT.APPLICANTMETHOD)='W') AND ((APPLICANT.ACADYEAR)='$re_acadyear') AND ((APPLICANT.SEMESTER)='$re_semester') AND ((APPLICANT.APPLICANTTYPE)='$re_type') AND ((APPLICANT.ROUND)='$re_round') AND ((APPLICANT.APPLICANTSTATUS)='5')) ORDER BY APPLICANT.CREATEDATETIME ASC) bc1  where rownum <= $goto ) where rnum > $pagelen2";
  ...
  ?>
	...
	<?   }  ?>
        ...
        <td bgcolor="#FFFFFF"><span class="style7">&nbsp;<? echo datethai($datetime); ?><br>
            <? echo $tel; ?></span></td>
        <td width="8%" bgcolor="#FFFFFF"><div align="center" class="style7"><? echo "<a href='$path_attachFile$docx' target='_blank'>ตรวจไฟล์</a>";?></div></td>
        <td width="8%" bgcolor="#FFFFFF"><div align="center" class="style7"><? echo "<a href='$path_attachFile$docy' target='_blank'>ตรวจไฟล์</a>";?></div></td>
        <td bgcolor="#FFFFFF"><div align="center"><span class="style7"><? echo $editseq; ?></span></div></td>
        <td bgcolor="#FFFFFF">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr><form action="index.php?p=frmcheckpass" method="post" name="form2" class="style7" id="form2">
              <td width="50%"><div align="center">
                <input name="imageField" type="image" src="../images/icon_true.png" alt="ผ่าน" />
							  <input name="lstacadyear" type="hidden" id="lstacadyear" value="<? echo $re_acadyear; ?>">
                              <input name="lstsemester" type="hidden" id="lstsemester" value="<? echo $re_semester; ?>">
                              <input name="lsttype" type="hidden" id="lstperiod" value="<? echo $re_type; ?>">
                              <input name="txt_round" type="hidden" id="lstlevel" value="<? echo $re_round; ?>">
                              <input name="check" type="hidden" id="check" value="yes">
							  <input name="applicantid" type="hidden" id="applicantid" value="<? echo $applicantid; ?>">
							  <input name="lstacadyear" type="hidden" id="lstacadyear" value="<? echo $re_acadyear; ?>">
							  <input name="lstsemester" type="hidden" id="lstsemester" value="<? echo $re_semester; ?>">
							  <input name="lsttype" type="hidden" id="lsttype" value="<? echo $re_type; ?>">
							  <input name="txt_round" type="hidden" id="txt_round" value="<? echo $re_round; ?>">
              </div></td></form>
              <td width="50%"><div align="center"><? echo "<a href='#' onClick=\"TINY.box.show({iframe:'problem.php?check=no&appid=$applicantid',boxid:'frameless',width:695,height:450,fixed:false,maskid:'bluemask',maskopacity:40,closejs:function(){closeJS()}})\"><img src='../images/icon_false.png' alt='ไม่ผ่าน' border='0'></a>";?></div></td>
            </tr>
          </table>        </td>
      </tr>
	  <? 
...
<p> <?
 ...
 echo "<strong>ทั้งหมด : </strong>  $totalrecords  คน/ จำนวน $totalpage หน้า <br/>"; 
	...
	{
		$back = $npage - 1;
		echo ""." <a href=\"$pathPresent?p=frmApplicant_check&npage=1&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round\" >แรกสุด</a> ";
		echo " <a href=\"$pathPresent?p=frmApplicant_check&npage=$back&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round\" >ก่อนหน้า</a> ";
		if ($start > 1) { echo "..."; }
	}
	...
	For ($i=$start ; $i<=$end ; $i++) 
	{
		if ($i == $npage ) 
		{
			...
			echo " <strong> $i </strong> ";
				 ...
				 echo " <a href=\"$pathPresent?p=frmApplicant_check&npage=$i&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round\"> $i </a> ";
		 	   }		   
	}
	...
	if ($npage < $totalpage) 
	{
		...
		echo " <a href='$pathPresent?p=frmApplicant_check&npage=$next&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round'>ถัดไป</a> ";
		echo " <a href='$pathPresent?p=frmApplicant_check&npage=$totalpage&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round' >ท้ายสุด</a> ";
	}
	?>
&nbsp;</p>
