...
<h2>พิมพ์เอกสารหลักฐานการสมัคร ระดับบัณฑิตศึกษาที่ชำระเงินแล้ว </h2>
<form id="form1" name="form1" method="post" action="index.php?p=frmApplicant_paytruephd">
  ปีการศึกษา 
  <select name="lstacadyear" id="lstacadyear">
    <option value="2561">2561</option>
    <?
                        $strSQL = "SELECT * FROM AVSREG.Z_ACADYEAR where id > 2559 ORDER BY id";
...
<select name="lstsemester" id="lstsemester">
 ...
 <select name="lsttype" id="lsttype">
   <?
                        $strSQL = "SELECT SYSBYTEDES.BYTECODE, SYSBYTEDES.BYTEDES FROM AVSREG.SYSBYTEDES WHERE (((SYSBYTEDES.TABLENAME)='APPLICANT') AND ((SYSBYTEDES.COLUMNNAME)='APPLICANTTYPE')) ORDER BY SYSBYTEDES.BYTECODE DESC ";
                        $result = odbc_exec($objConnect, $strSQL) or die ("Error Execute [".$strSQL."]");
                  		while(odbc_fetch_row($result))
							{
...
<table width="950" border="0" align="left" cellpadding="0" cellspacing="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="5">
      <tr>
        ...
        <td width="11%" bgcolor="#0099CC"><div align="center">ระดับ</div></td>
        <td width="11%" bgcolor="#0099CC"><div align="center">วันที่สมัคร</div></td>
        <td width="11%" bgcolor="#0099CC"><div align="center">วันที่ชำระเงิน</div></td>
	...
	$strSEL = "SELECT APPLICANT.APPLICANTID,APPLICANT.APPLICANTCODE, PREFIX.PREFIXNAME, APPLICANT.APPLICANTNAME, APPLICANT.APPLICANTSURNAME, APPLICANT.GPAX, APPLICANT.CREATEDATETIME, APPLICANT.APPLICANTMETHOD, APPLICANT.EDITSEQ FROM (((AVSREG.APPLICANT INNER JOIN AVSREG.PREFIX ON APPLICANT.PREFIXID = PREFIX.PREFIXID) INNER JOIN AVSREG.APPLICANTSELECTION ON APPLICANTSELECTION.APPLICANTID = APPLICANT.APPLICANTID) INNER JOIN AVSREG.QUOTASTATUS ON QUOTASTATUS.QUOTASTATUSID = APPLICANTSELECTION.QUOTASTATUSID) INNER JOIN AVSREG.VOUCHER ON VOUCHER.APPLICANTID = APPLICANT.APPLICANTID ";
	 $first_where = 0;
		 if($re_acadyear != "") 
		{	
			if($first_where == 0)
	...
	$strSEL .= " AND (((APPLICANT.APPLICANTMETHOD)='W') AND ((APPLICANT.APPLICANTSTATUS)>='10') AND ((APPLICANT.APPLICANTTYPE)='E') AND ((APPLICANT.FINANCESTATUS)='N') AND ((APPLICANTSELECTION.SEQUENCE)='1') AND ((QUOTASTATUS.STUDYPERIOD)<'3') AND ((VOUCHER.VOUCHERSTATUS)<>'V')) ORDER BY VOUCHER.VOUCHERDATE, APPLICANT.CREATEDATETIME ASC";	
	//echo $strSEL;	
	...
	$strSEL1 = "SELECT * from(select rownum rnum, bc1.* from(SELECT APPLICANT.APPLICANTID,APPLICANT.APPLICANTCODE, PREFIX.PREFIXNAME, APPLICANT.APPLICANTNAME, APPLICANT.APPLICANTSURNAME, APPLICANT.GPAX, APPLICANT.CREATEDATETIME, APPLICANT.HOMEPHONENO, APPLICANT.APPLICANTMETHOD, APPLICANT.EDITSEQ, VOUCHER.VOUCHERDATE FROM (((AVSREG.APPLICANT INNER JOIN AVSREG.PREFIX ON APPLICANT.PREFIXID = PREFIX.PREFIXID) INNER JOIN AVSREG.APPLICANTSELECTION ON APPLICANTSELECTION.APPLICANTID = APPLICANT.APPLICANTID) INNER JOIN AVSREG.QUOTASTATUS ON QUOTASTATUS.QUOTASTATUSID = APPLICANTSELECTION.QUOTASTATUSID) INNER JOIN AVSREG.VOUCHER ON VOUCHER.APPLICANTID = APPLICANT.APPLICANTID ";
	 $first_where = 0;
		 if($re_acadyear != "") 
		{	
			if($first_where == 0)
	...
	$strSEL1 .= " AND(((APPLICANT.APPLICANTMETHOD)='W') AND ((APPLICANT.APPLICANTSTATUS)>='10') AND ((APPLICANT.APPLICANTTYPE)='E') AND ((APPLICANT.FINANCESTATUS)='N') AND ((APPLICANTSELECTION.SEQUENCE)='1') AND ((QUOTASTATUS.STUDYPERIOD)<'3') AND ((VOUCHER.VOUCHERSTATUS)<>'V')) ORDER BY VOUCHER.VOUCHERDATE, APPLICANT.CREATEDATETIME ASC) bc1  where rownum <= $goto ) where rnum > $pagelen2";
	...
	
	{
		...
		$voucherdate = odbc_result($resultSEL1,"voucherdate");
  ...
  ?>
	...
	while($objSEL2 = odbc_fetch_row($resultSEL2))
	{
		...
		$levelid = odbc_result($resultSEL2,"levelid");	
  ...
  ?>
	...
	<?   }  ?>
        ...
        <td bgcolor="#FFFFFF"><span class="style7"><? if($levelid == '42') { echo "ปริญญาโท"; } 
														if($levelid == '50') { echo "ปริญญาเอก"; } ?></span></td>
        <td bgcolor="#FFFFFF"><span class="style7">&nbsp;<? echo datethai($datetime); ?><br><br>
        </span></td>
        <td bgcolor="#FFFFFF"><span class="style7">&nbsp;<? echo datethai($voucherdate); ?><br />
              <br />
        </span></td>
        ...
        <td bgcolor="#FFFFFF">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr><form action="index.php?p=frmApplicant_check" method="post" name="form2" class="style7" id="form2">
              <td><div align="center"><a href="frmPrint2phd.php?appid=<? echo $applicantid; ?>" target="_blank">พิมพ์</a></div></td>
            </form>
              </tr>
          </table>        </td>
      </tr>
...
<p> <?
 ...
 echo "<strong>ทั้งหมด : </strong>  $totalrecords  คน/ จำนวน $totalpage หน้า <br/>"; 
	...
	{
		$back = $npage - 1;
		echo ""." <a href=\"$pathPresent?p=frmApplicant_paytruephd&npage=1&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code\" >แรกสุด</a> ";
		echo " <a href=\"$pathPresent?p=frmApplicant_paytruephd&npage=$back&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code\" >ก่อนหน้า</a> ";
		if ($start > 1) { echo "..."; }
	}
	...
	For ($i=$start ; $i<=$end ; $i++) 
	{
		if ($i == $npage ) 
		{
			...
			echo " <strong> $i </strong> ";
				 ...
				 echo " <a href=\"$pathPresent?p=frmApplicant_paytruephd&npage=$i&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code\"> $i </a> ";
		 	   }		   
	}
	...
	if ($npage < $totalpage) 
	{
		...
		echo " <a href='$pathPresent?p=frmApplicant_paytruephd&npage=$next&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code'>ถัดไป</a> ";
		echo " <a href='$pathPresent?p=frmApplicant_paytruephd&npage=$totalpage&lstacadyear=$re_acadyear&lstsemester=$re_semester&lsttype=$re_type&txt_round=$re_round&txt_code=$re_code' >ท้ายสุด</a> ";
	}
	?>
&nbsp;</p>
